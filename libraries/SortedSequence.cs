...
using System.Collections.Generic;
    ...
    class SortedSequence<T> : List<T> where T : IComparable<T>
    {
        ...
        public static SortedSequence<T> initialiseSortedSequence(params T[] args)
        {
            SortedSequence<T> result = new SortedSequence<T>();
        ...
        public bool add(T x)
        {
            ...
            {
                int ip = -(insertionPoint + 1);
                elements.Insert(ip, x);
            }
            else
            {
                elements.Insert(insertionPoint, x);
            }
        ...
        public SortedSequence<T> unionSortedSequence(SortedSequence<T> sq)
        {
            ...
            {
                T obj = (T) elements[i];
                T sqelem = (T) sq.elements[reached];
        ...
        {
            int rem = elements.RemoveAll(col.Contains);
            return rem > 0;
        }
        ...
        {
            int removed = elements.RemoveAll(col.Contains);
            return removed > 0;
        }
        ...
        public bool equals(Object col)
        {
            if (col is SortedSequence<T>)
            {
                ...
                if (elements.Count != ss.elements.Count)
                {
                    return false;
                }
                ...
                for (int i = 0; i < elements.Count; i++)
                {
                    if (!(elements[i].Equals(ss.elements[i])))
                    {
                        return false;
                    }
                }
