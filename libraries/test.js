...
{ var col = new Map([[5,"bbb"], [3,"aaa"], [1, "tt"], [7,"cc"], [9, "ee"]]);
  var sub = new Map(); 
    // new Map([[7,"cc"], [5, "bbb"], [1, "tt"]]);  

  var sq = [1,4,5,7,5]; 
  var sq1 = [9,5]; 

  var chrs = "a Long String";

  var st = new Set(); 
  st.add(7); st.add(1); st.add(8); st.add(5); 

  var st1 = Ocl.asSet(sq1);

  var sq2 = ["jj", "ppp", "pa", "qqq", "p", "aa", "r"];  
  ...
  pp3.name = "Zeberdee";
  pp3.age = 33;  
  var persons = [pp1, pp3, pp2]; 

  var mm = Ocl.rejectMap(col, function(x) { return Ocl.size(x) <= 2; }); 
  // var ss = Ocl.includingSet(st, 3);
  return mm.get(5) + " " + mm.get(3);   
    // mm.get(5) + " " + mm.get(3) + " " + mm.get(1) + 
    //    " " + mm.get(7) + " " + mm.get(9); //
