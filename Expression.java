...
{ Expression javaForm;  
  ...
  public static Vector linearOperators = new Vector(); 
  static 
  { linearOperators.add("->indexOf"); 
    linearOperators.add("->lastIndexOf"); 
    linearOperators.add("->max"); 
    linearOperators.add("->min"); 
    linearOperators.add("->includes"); 
    linearOperators.add("->excludes"); 
    linearOperators.add("->select"); 
    linearOperators.add("->collect"); 
    linearOperators.add("->reject"); 
    linearOperators.add("->forAll");
    linearOperators.add("->exists"); 
  }  // and? 
 ...
 /* { if ((this + "").equals(conj + ""))
  ...
  { if (expr.hasBrackets())
    { expr.setBrackets(false); 
      boolean res = Expression.isValue("" + expr); 
  ...
  public static Expression simplifyExists(Expression col, 
                                          Expression pred)
  { // col->exists(false) is false 
    // col->exists(true) is col->notEmpty()
    // false for empty col
    ...
    { return Expression.simplifyNotEmpty(col,null); } 
  ...
  public static Expression simplifyForAll(Expression var, 
                                          Expression col, 
                                          Expression pred)
  ...
  { // sq->reverse()->min() is  sq->min()
    ...
    if (src instanceof UnaryExpression)
    { UnaryExpression usrc = (UnaryExpression) src; 
      String op = usrc.getOperator(); 
      if ("->reverse".equals(op) || "->asSet".equals(op) ||
          "->sort".equals(op) || "->asSequence".equals(op) ||
          "->oclAsSet".equals(op) ||
  ...
  { // Integer.subrange(1,n)->sum() is (n*(n+1))/2
    ...
    { BinaryExpression colexpr = 
      ...
      { System.err.println("!! OES flaw: " + var + " not used in " + expr);
        return new BinaryExpression("*", 
  ...
  { // Integer.subrange(1,n)->average() is (1+n)/2.0
    ...
    { UnaryExpression ue = (UnaryExpression) src; 
      ...
      if ("->reverse".equals(uop) || "->sort".equals(uop) || 
          "->asSet".equals(uop) || "->asSequence".equals(uop) ||
          "->oclAsSet".equals(uop) || 
          "->oclAsSequence".equals(uop))
      { System.err.println("!! OES flaw: redundant operator " + uop + " in " + src + "->average()");
    ...
    if (src instanceof BinaryExpression && 
        "->sortedBy".equals(
                 ((BinaryExpression) src).getOperator()))
    { BinaryExpression be = (BinaryExpression) src; 
      System.err.println("!! OES flaw: redundant ->sortedBy in " + src + "->average()");
    ...
    { BinaryExpression collexpr = (BinaryExpression) src; 
      ...
      System.err.println("!! OES: Redundant ->sortedBy in: " + src + "->average()"); 

      return Expression.simplifyAverage(srccoll); 
    } 
  ...
  { // sq->reverse()->sort()  is  sq->sort()
    ...
    return new UnaryExpression("->sort", src); 
  }
  ...
  { // Integer.subrange(1,n)->size() is n
    ...
    if (arg instanceof UnaryExpression) 
    { UnaryExpression ue = (UnaryExpression) arg; 
      String eop = ue.getOperator(); 
      if ("->sort".equals(eop) || 
          "->reverse".equals(eop) || 
          "->asSequence".equals(eop) ||
          "->oclAsSequence".equals(eop))
      { return Expression.simplifySize(ue.getArgument()); }
    } 
    ...
    { BinaryExpression bearg = (BinaryExpression) arg;
      String bop = bearg.getOperator(); 
 
      if ("->collect".equals(bop) || 
          "->sortedBy".equals(bop))
      { return simplifySize(bearg.getLeft()); }
      else if ("|C".equals(bop) || 
               "|sortedBy".equals(bop))
  ...
  public static Expression simplifySetAt(Expression src, 
                                         Expression arg1, 
                                         Expression arg2)
  { // sq.setAt(i,x) for i < 0  is  
    //    sq.setAt(sq->size() + i, x)
    // sq.setAt(i,x).setAt(i,y) is sq.setAt(i,y)
    ...
    if (src instanceof BasicExpression)
    { BasicExpression besrc = (BasicExpression) src; 
      if ("setAt".equals(besrc.getData()) && 
          par1.isEqualTo(besrc.getParameter(1)))
      { Vector vect = new Vector(); 
        vect.add(par1); 
        vect.add(par2); 
        return 
          BasicExpression.newFunctionBasicExpression(
                      "setAt", besrc.getObjectRef(), vect);
      } 
    } 
    ...
    { int ind1 = Expression.convertInteger(par1 + "");
      return SetExpression.setAt((SetExpression) src, 
                                 ind1, par2); 
    } 
  ...
  public static Expression simplifyExcludingAt(Expression src, 
                                         Expression arg1)
  { // sq->excludingAt(i) for i < 0  is  
    //    sq->excludingAt(sq->size() + i)
    // sq->excludingAt(1) is sq->tail()
    ...
    { int val1 = Expression.convertInteger(arg1 + ""); 
      ...
      if (val1 == 1) 
      { return Expression.simplifyTail(src); } 
    }
    ...
    if (Expression.isIntegerValue("" + par1) && 
        src instanceof SetExpression) 
    { int ind1 = Expression.convertInteger(par1 + "");
      return SetExpression.excludingAt((SetExpression) src, 
                                       ind1); 
    } 

    return new BinaryExpression("->excludingAt", src, par1);   
  }

  public static Expression simplifyInsertAt(Expression src, 
                                         Expression arg1, 
                                         Expression arg2)
  { // sq.insertAt(i,x) for i < 0  is  
    //    sq.insertAt(sq->size() + i, x)
    ...
    if (Expression.isIntegerValue("" + par1) && 
        src instanceof SetExpression) 
    { int ind1 = Expression.convertInteger(par1 + "");
      return SetExpression.insertAt((SetExpression) src, 
                                 ind1, par2); 
    } 
  ...
  public static Expression simplifyInsertInto(Expression src, 
                                         Expression arg1, 
                                         Expression arg2)
  { // sq.insertInto(i,x) for i < 0  is  
    //    sq.insertInto(sq->size() + i, x)
    // sq.insertInto(i, Sequence{})  is  sq
    ...
    if (Expression.isIntegerValue("" + par1) && 
        src instanceof SetExpression && 
        par2 instanceof SetExpression) 
    { int ind1 = Expression.convertInteger(par1 + "");
      return SetExpression.insertInto((SetExpression) src, 
                                 ind1, 
                                 (SetExpression) par2); 
    } 

    if (par2 instanceof SetExpression && 
        ((SetExpression) par2).size() == 0) 
    { return src; } 
  ...
  public static Expression simplifyExcludingSubrange(
                                            Expression src, 
                                            Expression arg1, 
                                            Expression arg2)
  { // sq.exlcudingSubrange(i,j)  for j < 0  is  
    //      sq.excludingSubrange(i, sq->size() + j)
    // sq.excludingSubrange(i,j)  for i < 0  is  
    //      sq.excludingSubrange(sq->size() + i, j)
    // sq->collect(x|e).excludingSubrange(a,b) is 
    //      sq.excludingSubrange(a,b)->collect(x|e)
    // sq.excludingSubrange(1, sq->size()) is Sequence{}
    // sq.excludingSubrange(i,i) is sq->excludingAt(i)
    ...
    { Expression res = new SetExpression(true); 
      res.setType(src.getType());
      res.setElementType(src.getElementType());  
      return res; 
    } 
    ...
    { int ind1 = Expression.convertInteger(par1 + "");
      ...
      String res1 = ("" + src).substring(0, ind1); 
      String res2 = ("" + src).substring(ind2); 
    ...
    if (par1.isEqualTo(par2))
    { return Expression.simplifyExcludingAt(src,par1); } 
    ...
    { int ind1 = Expression.convertInteger(par1 + "");
      ...
      return SetExpression.excludingSubrange(
                (SetExpression) src, ind1, ind2); 
    } 

    if (src instanceof BinaryExpression && 
        "->collect".equals(
              ((BinaryExpression) src).getOperator()))
    { // col->collect(expr).excludingSubrange(a,b) is 
      // col.excludingSubrange(a,b)->collect(expr)

      BinaryExpression be = (BinaryExpression) src;  
      Expression col = be.getLeft(); 
      Expression expr = be.getRight(); 
      Expression colrng = 
          Expression.simplifyExcludingSubrange(col, par1, par2);
      return new BinaryExpression("->collect", colrng, expr); 
    }   

    if (src instanceof BinaryExpression && 
        "|C".equals(
              ((BinaryExpression) src).getOperator()))
    { // col->collect(x|expr).excludingSubrange(a,b) is 
      // col.excludingSubrange(a,b)->collect(x|expr)

      BinaryExpression be = (BinaryExpression) src;  
      BinaryExpression indom = (BinaryExpression) be.getLeft();
      Expression var = indom.getLeft(); 
      Expression col = indom.getRight(); 
 
      Expression expr = be.getRight(); 

      Expression colrng = 
          Expression.simplifyExcludingSubrange(col, par1, par2);
      return Expression.simplifyCollect(colrng, var, expr); 
    }   
    ...
    pars.add(par2);  
    return BasicExpression.newFunctionBasicExpression(
                 "excludingSubrange", src, pars);   
  }

  public static Expression simplifySetSubrange(
                                      Expression src, 
                                      Expression arg1, 
                                      Expression arg2, 
                                      Expression val)
  { // sq.setSubrange(i, j, v)  for j < 0  is  
    //      sq.setSubrange(i, sq->size() + j, v)
    // sq.setSubrange(i,j,v)  for i < 0  is  
    //      sq.setSubrange(sq->size() + i, j, v)
    // sq.setSubrange(1, sq->size(), v) is v with sq type
    // sq.setSubrange(i,i,v) is sq.setAt(i,v)
    ...
    { BinaryExpression marg = (BinaryExpression) arg1; 
      ...
      if ("1".equals(rarg + "") && 
          Expression.isDecimalInteger(larg + ""))
      { int val1 = Expression.convertInteger(larg + "");
        if (val1 < 0)
        { Expression sze = 
            Expression.simplifySize(src);
          int actualval = (val1 + 1); 
          par1 = 
                new BinaryExpression("+", sze, 
                       new BasicExpression(actualval)); 
        }
    ...
    if ("1".equals(par1 + "") && 
        (src + "->size()").equals(par2 + "")) 
    { Expression res = (Expression) val.clone(); 
      res.setType(src.getType());
      res.setElementType(src.getElementType());  
      return res; 
    } 
    ...
    { int ind1 = Expression.convertInteger(par1 + "");
      ...
      BasicExpression expr = 
        BasicExpression.newValueBasicExpression(
             res1 + 
             sval.substring(1,sval.length()-1) + res2); 
      expr.setType(new Type("String", null)); 
      return expr; 
    } 
    ...
    if (par1.isEqualTo(par2))
    { return Expression.simplifySetAt(
                   src, par1, 
                   Expression.simplifyAny(val)); 
    } 
  ...
  { // sq.subrange(i,j)  for j < 0  is  
    ...
    if (src instanceof BinaryExpression && 
        "->collect".equals(
              ((BinaryExpression) src).getOperator()))
    { // col->collect(expr).subrange(a,b) is 
      // col.subrange(a,b)->collect(expr)

      BinaryExpression be = (BinaryExpression) src;  
      Expression col = be.getLeft(); 
      Expression expr = be.getRight(); 
      Expression colrng = 
          Expression.simplifySubrange(col, par1, par2);
      return new BinaryExpression("->collect", colrng, expr); 
    }   
    ...
    if (src instanceof BinaryExpression && 
        "|C".equals(
              ((BinaryExpression) src).getOperator()))
    { // col->collect(x|expr).subrange(a,b) is 
      // col.subrange(a,b)->collect(x|expr)
  ...
  { if (re instanceof SetExpression)
    ...
    { UnaryExpression ue = (UnaryExpression) re; 
      ...
      if ("->reverse".equals(uop) || "->sort".equals(uop) ||
          ...
          "->asSequence".equals(uop) || 
          "->oclAsSequence".equals(uop)) 
      { return simplifyIsEmpty(ue.getArgument(), vars); } 
    } 

    if (re instanceof BinaryExpression)
    { BinaryExpression ue = (BinaryExpression) re; 
      String uop = ue.getOperator(); 

      if ("->append".equals(uop) || 
          "->including".equals(uop) || 
          "->prepend".equals(uop))
      { return new BasicExpression(false); }

      if ("->sortedBy".equals(uop) || "->collect".equals(uop))
      { return simplifyIsEmpty(ue.getLeft(), vars); } 

      if ("|sortedBy".equals(uop) || 
          "|C".equals(uop))
      { BinaryExpression arg = 
                   (BinaryExpression) ue.getLeft();
        return simplifyIsEmpty(arg.getRight(), vars); 
      } 
    } 

    return new UnaryExpression("->isEmpty",re);
  }

  protected static Expression simplifyNotEmpty(
                                       Expression re,
                                       Vector vars)
  { if (re instanceof SetExpression)
    { SetExpression rse = (SetExpression) re;
    ...
    { UnaryExpression ue = (UnaryExpression) re; 
      ...
      if ("->reverse".equals(uop) || "->sort".equals(uop) ||
          ...
          "->asSequence".equals(uop) || 
          "->oclAsSequence".equals(uop)) 
      { return simplifyNotEmpty(ue.getArgument(), vars); } 
    } 
    ...
    { BinaryExpression ue = (BinaryExpression) re; 
      ...
      if ("->sortedBy".equals(uop) || "->collect".equals(uop))
      { return simplifyNotEmpty(ue.getLeft(), vars); } 

      if ("|sortedBy".equals(uop) || 
          "|C".equals(uop))
      { BinaryExpression arg = 
                   (BinaryExpression) ue.getLeft();
        return simplifyNotEmpty(arg.getRight(), vars); 
      } 
    } 

    return new UnaryExpression("->notEmpty",re);
  }
  ...
  { if (re instanceof SetExpression)
    ...
    { UnaryExpression ue = (UnaryExpression) re; 
      ...
      if ("->reverse".equals(uop) || "->sort".equals(uop) ||
          ...
          "->asSequence".equals(uop) || 
          "->oclAsSequence".equals(uop)) 
      { return simplifyIn(le, ue.getArgument(), vars); } 
    } 
    ...
    { BinaryExpression ue = (BinaryExpression) re; 
      ...
      if ("->sortedBy".equals(uop))
      { return simplifyIn(le, ue.getLeft(), vars); } 

      if ("|sortedBy".equals(uop))
      { BinaryExpression arg = 
                   (BinaryExpression) ue.getLeft();
        return simplifyIn(le, arg.getRight(), vars); 
      } 
    } 
  ...
  { if (re instanceof SetExpression)
    ...
    { UnaryExpression ue = (UnaryExpression) re; 
      ...
      if ("->reverse".equals(uop) || "->sort".equals(uop) ||
          ...
          "->asSequence".equals(uop) || 
          "->oclAsSequence".equals(uop)) 
      { return simplifyNotIn(le, ue.getArgument(), vars); } 
    } 
    ...
    { BinaryExpression ue = (BinaryExpression) re; 
      ...
      if ("->sortedBy".equals(uop))
      { return simplifyNotIn(le, ue.getLeft(), vars); } 

      if ("|sortedBy".equals(uop))
      { BinaryExpression arg = 
                   (BinaryExpression) ue.getLeft();
        return simplifyNotIn(le, arg.getRight(), vars); 
      } 
    } 
  ...
  { if (re instanceof SetExpression)
    ...
    { UnaryExpression ue = (UnaryExpression) re; 
      ...
      if ("->reverse".equals(uop) || "->sort".equals(uop) ||
          ...
          "->asSequence".equals(uop) || 
          "->oclAsSequence".equals(uop)) 
      { return simplifyIn(le, ue.getArgument()); } 
    } 
    ...
    { BinaryExpression ue = (BinaryExpression) re; 
      ...
      if ("->sortedBy".equals(uop))
      { return simplifyIn(le, ue.getLeft()); } 

      if ("|sortedBy".equals(uop))
      { BinaryExpression arg = 
                   (BinaryExpression) ue.getLeft();
        return simplifyIn(le, arg.getRight()); 
      } 
    } 
...
{ if (op1.equals("="))
 ...
 { Maplet mm = findSubexp(var);
  ...
  { /* if (Expression.isLong("10000000000000L"))
    ...
    Expression ss = new BasicExpression("\"abcdefg\""); 
    Expression smp = 
       Expression.simplifySubrange(ss, new BasicExpression(2),              
                                   new BasicExpression(4)); 
    System.out.println(smp); 

    Expression smp1 = 
       Expression.simplifyExcludingSubrange(
                          ss, new BasicExpression(2), 
                          new BasicExpression(4)); 
    System.out.println(smp1); 

    Expression smp2 = 
       Expression.simplifySetSubrange(
                          ss, new BasicExpression(2), 
                          new BasicExpression(4), 
                          new BasicExpression("\"xy\"")); 
