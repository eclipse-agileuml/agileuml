...
"Accept this?", "Imprecise: RHS <: LHS", JOptionPane.YES_NO_OPTION);
          ...
          { boolean foundsource = false; 
            ...
            { Vector srcJValues = new Vector(); 
              ...
              { /* JOptionPane.showInputDialog("*** Checking compositeSource2TargetTrees " + 
                ...
                if (amsub != null) 
                { /* JOptionPane.showInputDialog(">>-- Discovered mapping from subterm of source " + amsub); */ 
                  String fid = 
                    ...
                    // mapping, tim, and return _1 |-->_1`tim
                      ...
                      String tim = 
                         Identifier.nextIdentifier(
                                "singleElementxruleset");
                      TypeMatching tmnew1 = 
                          new TypeMatching(tim);
                      tmnew1.addValueMap(amjx1);     
                      tms.add(tmnew1);

                      BasicExpression tapp = 
                        new BasicExpression(tim); 
                      tapp.setUmlKind(Expression.FUNCTION);
                      tapp.addParameter(var1expr);

                      amjx = 
                        new AttributeMatching(var1expr, tapp);
                    }
                  }  
  ...
  { // Each strees[i] has same arity as ttrees[i]
    ...
    if (amjx != null) 
    { // JOptionPane.showInputDialog(">treesequence2>--- List function mapping: " + amjx); 
      ...
      } // also allow two+ symbols to be replaced by one
        // replacements may have duplicates, 
        // frequency(rs) == frequency(ds1) + frequency(ds2) 
      ...
      BasicExpression src1var = new BasicExpression("_1");
      ...
      BasicExpression pvar1expr = new BasicExpression(pid); 
      pvar1expr.setUmlKind(Expression.FUNCTION);
      pvar1expr.addParameter(new BasicExpression("_1")); */ 
      ...
      AttributeMatching amres = 
        ...
        // new AttributeMatching(src1var, pvar1expr);
        new AttributeMatching(srcstar, fexpr);   
      return amres; 
    } 
    return amjx; 
  } // Same style as seq3: amres is _1 |-->_1`fid
  ...
  { // ds is replaced consistently by rs in each 
    ...
    { ASTTerm st = strees[i]; 
      ...
      { return false; }  
    } // but could be >= if ds1, ds2 both replaced by rs
  ...
  { // Each strees[i] non-symbol element maps to 
    ...
    { String ds = (String) deletedSymbols.get(j); 
      ...
      } // as above, >= if ds1, ds2 both replaced by rs
    } 
  ...
  { // Each ttrees[i] non-symbol element originates from a 
    ...
    { String ds = (String) deletedSymbols.get(j); 
      ...
      } // >= if ds1, ds2 both replaced by rs
    } 
  ...
  { // Look for conditions on sent::ast attribute which 
    ...
    boolean foundConditionalMatching = false; 

    for (int i = 0; i < sarity; i++) 
    { if (ASTTerm.alwaysSymbol(i,srcasts)
          || 
          ASTTerm.alwaysBasic(i,srcasts))
      { System.out.println(">> Source term " + i + " is always a symbol/basic term"); 
        ...
        boolean allValuesHaveMapping = true; 
        ...
        { Vector dvals = new Vector(); 
          ...
          { String dval = (String) dvals.get(j); 
            ...
            else // try to subdivide the svalues,tvalues
            { Vector subres = 
                conditionalSubTreeMappings(sent,tent,svalues, 
                                           tvalues,tms); 
              if (subres != null && subres.size() > 0)
              { res.addAll(subres); } 
              else  
              { allValuesHaveMapping = false; }
            } 
          }
    ...
    { // instead, try conditions based on the tag of i'th term
      ...
      { if (ASTTerm.alwaysSymbol(i,srcasts))
        ...
        Vector svals = ASTTerm.allTagsAtIndex(i,srcasts); 
        System.out.println(">> Tag values at index " + i + ": " + svals);
        ...
        { // try splitting on this basis
          ...
          { String dval = (String) dvals.get(j); 
            ...
            for (int k = 0; k < nobjs; k++) 
            { if (ASTTerm.hasTagValue(srcasts[k],i,dval))
              { svalues.add(srcasts[k]); 
                tvalues.add(trgasts[k]); 
              } 
            } 
  ...
  public Vector conditionalSubTreeMappings(
      Entity sent, Entity tent, Vector sobjs, Vector tobjs,
      Vector tms) 
  { // Look for conditions on sent::ast attribute which 
    // partition the sobjs-->tobjs pairs that satisfy 
    ...
    for (int i = 0; i < nobjs; i++) 
    { ASTTerm trm = (ASTTerm) sobjs.get(i); 
      srcasts[i] = trm;
      ASTTerm ttrm = (ASTTerm) tobjs.get(i); 
      trgasts[i] = ttrm; 
    } 
