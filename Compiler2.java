...
/* The original RSDS used simple expressions involving =, =>, <=>, &, or. 
  ...
  Subsequently the notation has been extended to the OCL.g4 
  grammar available from the ANTLR grammar repository. 
...
{ static final int INUNKNOWN = 0; 
  ...
  public static String beforeComment(String s)
  { // extracts the substring before a one-line comment
    ...
    { char c = str.charAt(i); 
      ...
      { if (isSymbolCharacter(c))
        ...
        { sb = new StringBuffer();     // unrecognised lexical
          ...
          { System.out.println("!! Unrecognised UML/OCL token: " + c); } 
        }
      } 
      ...
      { if (isBasicExpCharacter(c) || 
        ...
        { sb = new StringBuffer();     // unrecognised lexical
          lexicals.addElement(sb);  
          in = INUNKNOWN; 
          // System.out.println("!! Unrecognised token in expression: " + c); 
          sb.append(c); 
        }
      }
      else if (in == INSYMBOL)
      ...
      { if (c == '"' && prev != '\\')
        { sb.append('"'); 
          if (stringDelimiter == '"')  // ends a " string
          { in = INUNKNOWN; } 
        } 
        else if (c == '\'' && prev != '\\')
        { if (stringDelimiter == '\'')
          { in = INUNKNOWN; 
            sb.append('"');   // ends a ' string
          }
          else 
          { sb.append("\'"); } // ' within a " string
        } 
        else 
        { sb.append(c); } 
      }    
  ...
  { if (st > en) { return null; }
    ...
    { Type tt = null; 
        ...
        { for (int i = st + 2; i < en; i++) 
          { String ss = lexicals.get(i) + ""; 
            ...
            { Type t1 = parseType(st+2,i-1,entities,types); 
              ...
              { System.out.println("!! Invalid type at: " + showLexicals(st+2,i-1)); } 
              else if (t2 == null) 
              { System.out.println("!! Invalid type at: " + showLexicals(i+1,en-1)); } 
            }
          } 
        }
      } 
      ...
      if (tt == null) 
      { System.out.println("!!ERROR!!: Invalid map/function type, it must have 2 type arguments: " + showLexicals(st,en)); 
        tt = new Type(typ, null);
        tt.setKeyType(new Type("OclAny", null)); 
        tt.setElementType(new Type("OclAny", null)); 
        return tt;  
    ...
    { Type tt = null; 
      if (st == en) 
      { System.out.println("!! Warning, map types must have type parameters"); 
      ...
      if (tt == null) 
      { System.out.println("!!ERROR!!: Invalid map type, it must have 2 type arguments: " + showLexicals(st,en)); 
        tt = new Type("Map", null);
        tt.setSorted(true); 
        tt.setKeyType(new Type("OclAny", null)); 
        tt.setElementType(new Type("OclAny", null)); 
    ...
    { Type tt = new Type(typ,null);
      ...
      { String ob = lexicals.get(st+1) + ""; 
        ...
        { Type et = parseType(st + 2, en - 1, entities, types); 
          ...
          { System.out.println("!! Error: Unknown type in Set/Sequence argument: " + showLexicals(st+2,en-1)); } 
        } 
      }  
      // return tt; 
    } 
  ...
  public Expression parsePrimitiveExpression() 
  { System.out.println("LEXICALS: " + lexicals);
    if (lexicals.size() == 0)
    { return null; } 
 
    Vector env = new Vector(); 
    Expression ee = parse_primitive_expression(0,0,lexicals.size()-1,env,env); 
    finalexpression = ee; 
    return ee; 
  } 
 ...
 /*  public Expression parse_expression(int bcount, int pstart, int pend)
  ...
  { Expression ee = null; 
    ...
    if (lexicals.size() == 0 || pstart > pend || 
        pend >= lexicals.size()) 
    { return ee; } 
    
    if ("if".equals(lexicals.get(pstart) + "") && 
        "endif".equals(lexicals.get(pend) + ""))
    ...
    // System.out.println(">>> Best op: " + op + " " + bcount); 
...
{ // st is "if", en is "endif"
  ...
  { String lxj = lexicals.get(j) + "";
    ...
    { ifcount--; 
      if (ifcount < 1)
      { System.out.println("!!! Too many endifs in conditional"); 
        return null; 
      }
    }
    ...
    { Expression test = parse_expression(bc, st+1, j-1, entities, types);
     if (test == null)
     { System.out.println("!! Error in if test expression"); return null;  }
      int ifcountk = 1;
      ...
      { String lxk = lexicals.get(k) + "";
        ...
        { ifcountk--;  
          if (ifcountk < 1)
          { System.out.println("Too many endifs"); return null; }
        }
...
{ // st is "if", en is "endif"
  ...
  { String lxj = lexicals.get(j) + "";
    ...
    { ifcount--; 
      if (ifcount < 1)
      { System.out.println("!! Error: Too many endifs"); 
        return null;
      }
    }
    else if ("then".equals(lxj) && ifcount == 1)
    { Expression test = parse_ATLexpression(bc, st+1, j-1,entities,types);
      ...
      { System.out.println("!! Error in if test expression"); return null;  }
       ...
       { String lxk = lexicals.get(k) + "";
         ...
         { ifcountk--;  
           if (ifcountk < 1)
           { System.out.println("!! Error: Too many endifs"); return null; }
         }
         else if ("else".equals(lxk) && ifcountk == 1)
...
{ // let v : T = e in expr
  ...
  { String lxj = lexicals.get(j) + "";
    ...
    { if (letcount > 1)  // an inner let
      ...
      { System.out.println("!! Too many let in clauses"); 
        return null;
      }
      else 
      { Expression var = parse_ATLexpression(bc, st+1, st+1,entities,types);
        if (var == null)
        { System.out.println("!! Error in let variable"); return null;  }
        ...
        { String lxk = lexicals.get(k) + "";
          ...
          { Type ltype = parseType(st+3,k-1,entities,types);
            if (ltype == null)
            { System.out.println("!! Error in let type"); 
              return null;
            }
            ...
            if (lexp == null)  
            { System.out.println("!! Error in let definition expression"); 
              return null;
            }
            ...
            if (lbody == null)
            { System.out.println("!! Error in let body"); }
...
{ // lambda v : T in expr
  ...
  System.out.println("*** Parsing lambda: " + showLexicals(st,en)); 
  ...
  { String lxj = lexicals.get(j) + "";
    ...
    { if (letcount > 1)  // an inner let
      ...
      { System.out.println("!! Error: Too many 'in' clauses in lambda expression"); 
        return null; 
      }
      else 
      { Expression var = parse_primitive_expression(bc, st+1, st+1,entities,types);
  
        if (var == null)
        { System.out.println("!! Invalid syntax of lambda variable: " + lexicals.get(st+1)); 
          return null;  
        }
        ...
        if (ltype == null)
        { System.out.println("!! Error in lambda type: " + showLexicals(st+3,j-1)); 
          ltype = new Type(lexicals.get(st+3)+"", null); 
        }

        System.out.println("*** Parse expression: " + showLexicals(j+1,en)); 

        Expression lbody = 
           parse_expression(bc+1,j+1,en,entities,types);
 
        if (lbody == null)
        { System.out.println("!! Error in lambda body expression: " + showLexicals(j+1,en));
 
          Statement lcode = parseStatement(j+1, en, entities, types); 
          System.out.println(">> Statement: " + lcode);

          if (lcode == null) 
          { continue; } 

          lbody = BehaviouralFeature.operationFromLambdaStatements(
                                     lcode, entities, 
                                     var, ltype);  
        }
  ...
  { // There must be a logical operator at top level in the expression
    ...
    // if (e1 == null) 
    // { System.out.println("!! Invalid expression: " + showLexicals(pstart, pos-1)); }
 
    e2 = parse_expression(bcount,pos+1,pend,env,env); 

    // if (e2 == null) 
    // { System.out.println("!! Invalid expression: " + showLexicals(pos + 1, pend)); }
    ...
    { String ss = lexicals.get(i).toString(); 
      ...
      { String ss2 = lexicals.get(i+1).toString(); // must exist
        ...
        { Expression ee2 = parse_factor_expression(bc,pstart,i-1,entities,types);
          ...
          if (ee2 == null) 
          { // System.out.println("!! Invalid unary -> expression at: " + showLexicals(pstart,pend)); 
            continue; 
          }
  ...
  { 
    ...
    { String ss = lexicals.elementAt(pstart).toString(); 
      ...
      { // System.out.println("!!! ERROR: Invalid basic expression: " + ss); 
        return null; 
      } 

      if (cstlVarPatt.matcher(ss).find())
      { System.out.println("!! Warning: do not use identifiers _i for integers i: " + ss); }

      /* if (isKeyword(ss))
      { System.out.println(">>>: Invalid basic expression: keyword: " + ss); 
        return null;
      } */ 
    ...
    if (pstart + 2 < pend && "let".equals(lexicals.get(pstart) + ""))
    ...
    { Expression arg = parse_additive_expression(bc,pstart+1,pend,entities,types); 
      if (arg == null) 
      { // System.out.println("!! Not additive expression: " + showLexicals(pstart+1, pend)); 
        return null; 
      } 
    ...
    { Expression arg = parse_basic_expression(bc,pstart+1,pend,entities,types); 
      if (arg == null) 
      { System.out.println("!! Error: references must be applied to basic expressions: " + showLexicals(pstart+1, pend)); 
        return null; 
      } 
    ...
    { Expression arg = parse_basic_expression(bc,pstart+1,pend,entities,types); 
      if (arg == null) 
      { System.out.println("!! Error: dereferences must be applied to basic expressions: " + showLexicals(pstart+1, pend)); 
        return null; 
      } 
    ...
    { // op(pars)
      ...
      Expression op = parse_primitive_expression(bc,pstart,pstart,entities,types); 
  ...
  /*
    ...
    { // iden.op(args)
      ...
      Expression objref = parse_primitive_expression(bc,pstart,pstart,entities,types); 
    ...
    if (pstart < pend && 
        "]".equals(lexicals.get(pend) + "") && 
        "[".equals(lexicals.get(pstart+1) + "") && 
        balancedBrackets(pstart+1, pend))
    { // iden[indexes]
      ...
      { 
        ...
        else if (op instanceof BasicExpression) // must be
        { BasicExpression beop = (BasicExpression) op;  
    ...
    if (pstart < pend && 
        ...
        balancedBrackets(pstart+1, pend-1))  
    { // be[inds].f
      ...
      { 
        ...
        if (arg == null) 
        { // System.out.println("!! ERROR: Invalid array argument: " + showLexicals(pstart+2,pend-2)); 
    ...
    if (pstart+1 < pend &&
        ...
        ")".equals(lexicals.get(pend-1) + "") &&
        "(".equals(lexicals.get(pstart) + "") && 
        balancedBrackets(pstart, pend-1))  
    { String tail = lexicals.get(pend) + ""; 
      if (tail.charAt(0) == '.')
      { tail = tail.substring(1,tail.length()); 
        ...
        { BasicExpression beop = (BasicExpression) op;  
          ...
          return ef;  
                 ...
                 op instanceof UnaryExpression && 
                 validObjectRefOperator(((UnaryExpression) op).operator))
        { Expression ef = te.checkIfSetExpression(); 
          if (ef != null && (ef instanceof BasicExpression)) 
          ...
          return ef;  
                 ...
                 op instanceof BinaryExpression && 
                 validObjectRefOperator(((BinaryExpression) op).operator))
        { Expression ef = te.checkIfSetExpression(); 
          if (ef != null && (ef instanceof BasicExpression)) 
          { ((BasicExpression) ef).setInnerObjectRef(op); }  
          return ef;  
  ...
  { // (be).op(pars)
    ...
    { String brack = lexicals.get(i-1) + ""; 
      ...
      { Expression op = parse_basic_expression(bc, pstart+1, i-2, entities,types);
        ...
        System.out.println("*** Parsing extended basic expression " + op + strs + "(" + ve + ")");

        if (op instanceof BasicExpression)
        { BasicExpression beop = (BasicExpression) op;
  ...
  return parse_primitive_expression(bc, pstart, pend, entities, types); 
}

  public static boolean validObjectRefOperator(String op)
  { // can be used before .feature chain
    return "!".equals(op) || 
       "->first".equals(op) || 
       "->last".equals(op) ||  
       "->at".equals(op) || 
       "->any".equals(op) || 
       "|A".equals(op); 
  } // others such as ->max, ->min, 
    // ->front, ->tail for strings
  
  public Expression parse_primitive_expression(int bc, 
       int pstart, int pend, Vector entities, Vector types)
  { // a sequence of identifiers with parameters or 
    // array indexes only
    ...
    if (pstart < pend &&  
        (lexicals.get(pend) + "").charAt(0) == '.') 
    { // be.f  valid primitive expression before .f

      Expression op = parse_primitive_expression(
                      bc,pstart,pend-1,entities,types);
      ...
      { BasicExpression beop = (BasicExpression) op;  
        BasicExpression rees = new BasicExpression(tail, 0); 
        rees.setObjectRef(beop); // InnerObjectRef  
        System.out.println(">>> Parsed primitive expression: " + rees);
        return rees;  
      }
    ...
    if (pstart + 1 < pend &&  
        "]".equals(lexicals.get(pend) + "")) 
    { // be [ ind ]  valid primitive expression be

      for (int k = pend-1; k > pstart; k--)
      { String lex = lexicals.get(k) + ""; 
        if ("[".equals(lex) && balancedBrackets(k,pend))
        { Expression op = parse_primitive_expression(
                            bc,pstart,k-1,entities,types); 
      
          if (op == null || "null".equals(op + "")) 
          { return null; } // no alternative 
          ...
          { BasicExpression beop = (BasicExpression) op;  
            ...
            if (beop.arrayIndex == null)
            { beop.setArrayIndex(arg); 
              System.out.println(">>> Parsed array 3 primitive expression: " + beop);
              return beop;
            } 
            
            if (beop.arrayIndex != null)
            { BinaryExpression res = 
                      new BinaryExpression("->at", beop, arg); 
    ...
    { // be.op(pars)
      ...
      { String lex = lexicals.get(k) + ""; 
        ...
        if ("(".equals(lex) && balancedBrackets(k,pend))
        { Expression op = parse_primitive_expression(bc, pstart, k-1, entities,types);

          if (op == null || "null".equals(op + "")) 
          { return null; } // no alternative 

          System.out.println("*** Parsing extended primitive expression " + op + "(...)");

          if (op instanceof BasicExpression)
          { Vector ve = 
              parse_fe_sequence(bc,k+1,pend-1,entities,types);
            ...
            beop.setIsEvent();
            beop.setParameters(ve);
            System.out.println("*** Parsed call primitive expression " + beop);
            return beop;     
          }
        }
      }
  ...
  { // Scans the lexicals inside the outer brackets of 
    // a potential fe-sequence
     ...
     { String lx = lexicals.get(i) + "";
       ...
       { if (bcnt == 0 && sqbcnt == 0 && cbcnt == 0)
         { // top-level ,
           ...
           // if (exp == null) 
           // { exp = parse_lambda_expression(bc,st0,i-1,entities,types); } 
     ...
     { Expression exp = 
       ...
       if (exp == null) 
       { System.out.println("!! Invalid additive exp: " + buff);
         exp = parse_basic_expression(bc,st0,en,entities,types); 
       } 

       if (exp == null) 
       { System.out.println("!! Invalid additive/lambda expression: " + showLexicals(st0,en));
         return null;
       }
     ...
     { System.out.println("!! Not fe sequence: " + showLexicals(st,en)); 
       return null;
     }
  ...
  { // Scans the lexicals inside the outer brackets of 
    // a potential fe-sequence
     ...
     { System.out.println("!! Not map element sequence: " + showLexicals(st,en)); 
       return null;
     }
     return res; 
   }
...
{ // name<typepars>(pars) : returnType 
  ...
  { String lx = lexicals.get(i) + "";
    ...
    { Expression pre = parse_expression(0,st0,i-1,localEntities,types);
      if (pre == null)
      { System.out.println("!! ERROR: Invalid precondition: " + showLexicals(st0,i-1)); 
      ...
      { String jx = lexicals.get(j) + ""; 
        ...
        { st0 = j+2; 
          ...
          if (pots == null) 
          { System.out.println("!! ERROR: Invalid postcondition syntax: " + showLexicals(i+2,j-1));
  ...
  if (pst == null)
  { System.out.println("!! ERROR: Invalid postcondition: " + showLexicals(st0,en));
    bf.setPost(new BasicExpression(true)); 
  }
  else 
  { bf.setPost(pst); 
  ...
  if (foundpost == false)
  { System.out.println("!! Invalid operation definition, no postcondition: " + showLexicals(st,en)); 
...
{ int bcnt = 0; int st0 = st+1;
  ...
  { String lx = lexicals.get(j) + "";
    ...
    { bcnt--; 
      if (bcnt < 0)
      { System.out.println("!! Extra closing bracket in declaration: " + showLexicals(st,j)); 
        return;
      }
    } 
    ...
    { String attnme = lexicals.get(j-1) + ""; 
      ...
      { String lxk = lexicals.get(k) + ""; 
        ...
        { att = parseParameterDec(j-1, k-3, entities, types);     
          if (att == null)
          { System.out.println("*** Invalid parameter syntax: " + showLexicals(st0,j-1)); }
          else
    ...
    { Type rt = parseType(j+1,en,entities,types);
      if (rt == null)
      { System.out.println("!! ERROR: Invalid return type: " + showLexicals(j+1, en)); 
        bf.setQuery(false); 
  ...
  if (bcnt > 0)
  { System.out.println("!! Unclosed bracket in declaration: " + showLexicals(st,en)); }
}
...
{ // should be att : Type
  ...
  if (typ == null)
  { System.out.println("!! ERROR: Invalid/unknown parameter type: " + showLexicals(st+2, en)); 
    return null; 
  }
...
{ // can be att : Type = value
  ...
  { String lex = lexicals.get(i) + ""; 
    ...
    { Expression val = parse_expression(0,i+1,en,entities,types); 
      ...
      if (typ == null)
      { System.out.println("!! ERROR: Invalid/unknown type: " + showLexicals(st+2, i-1)); 
        return null; 
      }
  ...
  if (tt == null)
  { System.out.println("!! ERROR: Invalid/unknown type: " + showLexicals(st+2, en)); 
    return null; 
  }
...
{ int n = lexicals.size(); 
 ...
 /*  public Expression parse_call_expression(int pstart, int pend)
  ...
  { // System.out.println("Trying tp parse basic expression from " + pstart + " to " + pend); 
    ...
    { String ss = lexicals.elementAt(pstart).toString(); 
      if (invalidBasicExp(ss))
      { System.out.println("!! Invalid ATL basic expression: " + ss); 
        return null; 
      } 

      if (isKeyword(ss))
      { System.out.println("!! Invalid ATL basic expression: keyword: " + ss); 
        return null;
      } 
    ...
    { Expression arg = parse_ATLadditive_expression(bc,pstart+1,pend,entities,types); 
      if (arg == null) 
      { // System.out.println("! Not ATL additive expression: " + showLexicals(pstart+1, pend)); 
        return null; 
      } 
      return new UnaryExpression("-",arg); 
    } // likewise for "not"
    ...
    { String tail = lexicals.get(pend) + ""; 
      ...
      if (arg == null) 
      { System.out.println("!! Invalid ATL array argument: " + showLexicals(pstart+2,pend-2)); } 
  ...
  { // Scans the lexicals inside the outer brackets of 
    // a potential fe-sequence
     ...
     { System.out.println("!! ERROR: Not ATL factor expression sequence: " + showLexicals(st,en)); 
       return null;
     }
     return res; 
   }
  ...
  { int ob = 0;
    ...
    { String lex = lexicals.get(i) + ""; 
      ...
      { cb++; 
        ...
        if (cb > ob)
        { System.out.println("!! Too many ) brackets (opening: " + ob + ", closing: " + cb + ") here->   " + showLexicals(st,i)); }

        if (mostrecentopen != null && !mostrecentopen.equals("("))
        { System.out.println("!! Error: closing " + mostrecentopen + " with ) here->   " + showLexicals(st,i)); 
        }
        else 
        { mostrecentopen = null; } 
      }
      ...
      { csqb++; 
        ...
        if (csqb > osqb)
        { System.out.println("!! Too many ] brackets (opening: " + osqb + ", closing: " + csqb + ") here->   " + showLexicals(st,i)); }
      ...
      { ccb++; 
        ...
        if (ccb > ocb)
        { System.out.println("!! Too many } brackets (opening: " + ocb + ", closing: " + ccb + ") here->   " + showLexicals(st,i)); }
    ...
    if (ob > cb)
    { System.out.println("!! Too many ( brackets (opening: " + ob + ", closing: " + cb + ") in   " + showLexicals(st,en)); }
    else if (cb > ob) 
    { System.out.println("!! Too many ) brackets (opening: " + ob + ", closing: " + cb + ") in   " + showLexicals(st,en)); }

    if (osqb > csqb)
    { System.out.println("!! Too many [ brackets (opening: " + osqb + ", closing: " + csqb + ") in   " + showLexicals(st,en)); }
    else if (csqb > osqb) 
    { System.out.println("!! Too many ] brackets (opening: " + osqb + ", closing: " + csqb + ") in   " + showLexicals(st,en)); }

    if (ocb > ccb)
    { System.out.println("!! Too many { brackets (opening: " + ocb + ", closing: " + ccb + ") in   " + showLexicals(st,en)); }
    else if (ccb > ocb) 
    { System.out.println("!! Too many } brackets (opening: " + ocb + ", closing: " + ccb + ") in   " + showLexicals(st,en)); }
  ...
  { 
    ...
    { mess[0] = "Unbounded loop statement:\n" + 
    ...
    { mess[0] = "Bounded loop statement:\n" + 
                " for variable : collection do code\n" + 
                " The variable must not already be defined, and cannot be written in the code.\n" + 
                " This statement can only be used in an activity"; 
      return "for"; 
    }

    if ("var".startsWith(st)) 
    { mess[0] = "Declaration/creation statement:\n" +  
        " var variable : Type\nCan only be used in an activity"; 
      return "var"; 
    } 
    ...
    { mess[0] = "Query operation declaration,\n returning a result value:\n" + 
    ...
    { mess[0] = "Operation precondition,\n" + 
                " e.g: pre: true\n" + 
                "Or usecase precondition, e.g.: precondition par > 0;\n"; 
      return "pre: expr   or    precondition expr;"; 
    } 
 

    if ("let".startsWith(st))
    { mess[0] = "let expression:\n" + 
                " let x : Type = init in (expr)"; 
      return "let"; 
    } 
 
    if ("activity".startsWith(st)) 
    { mess[0] = "Activity of operation or usecase\n" + 
                " activity: statement;"; 
      return "activity:"; 
    }
 
    if ("class".startsWith(st)) 
    { mess[0] = "Class declaration,\n" + 
                " e.g: class Name { ... }"; 
      return "class"; 
    } 

    if ("abstract".startsWith(st)) 
    { mess[0] = "Abstract class or operation declaration,\n" + 
        " e.g.: abstract class Name { ... }\n  abstract operation op() pre: true post: true;"; 
      return "abstract class/operation"; 
    } 

    if ("usecase".startsWith(st)) 
    { mess[0] = "usecase declaration,\n" + 
                " e.g.: usecase name : Type { ... }"; 
      return "usecase"; 
    } 
    ...
    { mess[0] = "attribute declaration,\n" + 
                " e.g.: attribute name : Type; or\n" + 
    ...
    { mess[0] = "assert statement,\n" + 
                " e.g.: assert condition do message;"; 
      return "assert"; 
    }
    ...
    { if ("else".startsWith(st)) 
      ...
      { mess[0] = "Operation postcondition,\n" + 
                  " e.g.: post: true\n" + 
      ...
      { mess[0] = "usecase parameter,\n" + 
                  " e.g.: parameter name : Type;"; 
        return "parameter"; 
      } 
  ...
  { /* boolean balanced = checkBrackets(s,e); 
    if (!balanced) 
	{ System.out.println("Not statement -- different number of ( and ) brackets"); 
	  return null; 
	} */ 
  ...
  { if ("for".equals(lexicals.get(s) + ""))
    { Statement s1 = null;
      ...
      { String ss = lexicals.get(i) + "";
        ...
        { test = parse_statement_expression(0,s+1,i-1,entities,types);
          if (test == null) 
          { System.out.println("!! ERROR: Invalid test in for loop: " + showLexicals(s+1,i-1)); 
            return null; 
          } 
      ...
      // System.out.println("Unable to parse for statement: " + lexicals.get(s) + " ... " + 
      //                    lexicals.get(e)); 
    }
    ...
    { Statement s1 = null;
        ...
        { test = parse_statement_expression(0,s+1,i-1,entities,types);
          ...
          { if ("(".equals(lexicals.get(s+1) + "") && 
            ...
            { test = parse_statement_expression(0,s+2,i-2,entities,types); 
              if (test == null)
              { System.out.println("ERROR!!: Invalid test in while loop: " + showLexicals(s+1,i-1));
      ...
      // System.out.println("Unable to parse while statement: " + lexicals.get(s) + " ... " + 
      //                    lexicals.get(e)); 
    }
        ...
        { s1 = parseStatement(s+1, i-1, entities, types);
          ...
          { if ("(".equals(lexicals.get(i+1) + "") && 
            ...
            { test = parse_statement_expression(0,i+2,e-2,entities,types); 
              if (test == null)
              { System.out.println("!!ERROR!!: Invalid test in while loop: " + showLexicals(i+1,e-1));
                return null; // continue; ???
              } 
              test.setBrackets(true); 
            }  
  ...
  { if ("if".equals(lexicals.get(s) + ""))
    { Statement s1 = null;
      ...
      { String ss = lexicals.get(i) + "";
        ...
        { test = parse_statement_expression(0,s+1,i-1,entities,types);
          if (test == null) 
          { System.out.println("!! ERROR: Invalid test in conditional: " + showLexicals(s+1,i-1)); 
            continue; 
          } 
      ...
      // System.out.println("Unable to parse if-then-else statement: " + showLexicals(s,e));  
    }
    return null;
    ...
    { Statement body = null;
      ...
      { String ss = lexicals.get(i) + "";
        ...
        { body = parseStatement(s+1,i-1,entities,types);
          if (body == null) 
          { // System.out.println("!! ERROR: Invalid body in try statement: " + showLexicals(s+1,i-1)); 
            return null; 
          } 
      ...
      { String ss1 = lexicals.get(j) + "";
        ...
        { Statement cs1 = parseBasicStatement(currentStart, j-1, entities, types);
          if (cs1 == null) 
          { // System.out.println("!! ERROR: Invalid catch/finally in try statement: " + showLexicals(currentStart,j-1)); 
            return res; 
          }
          res.addClause(cs1);  
          currentStart = j; 
      ...
      { Statement cs2 = parseBasicStatement(currentStart,e,entities,types); 
        if (cs2 == null) 
        { // System.out.println("!! ERROR: Invalid catch/finally in try statement: " + showLexicals(currentStart,e)); 
          return res; 
        }
        res.addClause(cs2);  
      }
  ...
  { /* skip, return, continue, break, error, catch, var, execute, 
    ...
    { if ("skip".equals(lexicals.get(e) + ""))
      ...
      { Expression ee = parse_basic_expression(0,s,e,entities,types);
        if (ee == null) 
        { // System.out.println("!! Invalid basic expression: " + showLexicals(s,e)); 
          return null; 
        } 
    ...
    { for (int p = s+2; p < e; p++) 
      ...
      // System.out.println("!! Invalid catch statement: " + showLexicals(s,e));     
    } 
    ...
    { for (int p = s+1; p < e; p++) 
      ...
      // System.out.println("!! Invalid assert statement: " + showLexicals(s,e));     
    } 
    ...
    { Expression ret = parse_expression(0,s+1,e,entities,types); 
      ...
      /* else 
      { System.out.println("!! Invalid expression in error statement: " + showLexicals(s+1,e)); } */     
    } 
    ...
    { // creation with complex type
      ...
      { System.out.println("!! Warning: unknown type in var statement: " + showLexicals(s+3,e));
 ...
 /*   else if (e == s + 2 && ":".equals(lexicals.get(s+1) + ""))
    { CreationStatement cs = new CreationStatement(lexicals.get(e) + "", lexicals.get(s) + "");
      ...
      { System.out.println("!! Warning: Unknown type in : statement: " + lexicals.get(e)); }  
      return cs; 
    } */ // allow any type
    else 
    { for (int i = s; i < e; i++) 
  ...
  { Vector res = new Vector(); 
    ...
    { String ss = lexicals.get(i) + "";
      ...
      { // Top-level statement
        s1 = parseStatement(s,i-1,entities,types);
        if (s1 == null) 
        { System.out.println("WARNING: unrecognised statement: " + showLexicals(s,i-1)); 
          continue; 
        } 
        else
  ...
  { String rname = ""; 
    ...
    { System.out.println("!! Invalid classifier keyword: " + lx); 
      return null; 
    } 
  ...
  { boolean abstr = false; 
    ...
    { supr = lexicals.get(start+1) + ""; 
      ...
      System.out.println(">>> " + rname + " specialises " + supr); 
        ...
        { if ("static".equals(lxr))
          { if ("attribute".equals(
            ...
            { Attribute attr = 
              ...
              if (attr == null) 
              { System.out.println("!! Cannot parse attribute " + 
            ...
            { BehaviouralFeature bf = 
              ...
              { System.out.println("!! ERROR: Invalid static operation definition: " + 
          ...
          { // JOptionPane.showInputDialog("Abstract operation " + lexicals.get(reached+1)); 
            ...
            { BehaviouralFeature bf = 
              ...
              { System.out.println("!! ERROR: Invalid abstract operation definition: " + 
                               showLexicals(reached+2, i-2));
                Vector error3 = new Vector(); 
                error3.add("Invalid operation declaration"); 
                error3.add(showLexicals(reached+2,i-2)); 
          ...
          { PreAssociation ast = parseAssociationClause(reached, i-1, entities, types); 
            if (ast == null) 
            { System.out.println("!! Cannot parse reference " + 
                                   lexicals.get(reached + 1));
          ...
          { BehaviouralFeature bf = 
            ...
            { System.out.println("!! ERROR: Invalid operation definition: " + 
          ...
          { Expression expr = parse_expression(0,reached+1,i-2,entities,types); 
            ...
            { System.out.println("!! Invalid invariant expression: " + showLexicals(reached+1,i-2)); 
          ...
          { String stereo = ""; 
	       ...
	       { // System.out.println(lexicals.get(k) + ""); 
			...
			{ if (expr instanceof BasicExpression) 
			  ...
			  { System.out.println("!! Unexpected form of stereotype expression: " + expr); }
			} */ 
			// else 
			// { System.out.println("!! Invalid stereotype expression: " + showLexicals(reached+1,i-2)); 
			//   checkBrackets(reached+1,i-2); 
			//   Vector error5 = new Vector(); 
    ...
    { boolean isStatic = false; 
      ...
      { Expression expr = parse_expression(0,reached+1,en-2,entities,types); 
        ...
        { System.out.println("!! Invalid invariant expression: " + showLexicals(reached+1,en-2)); 
	  ...
	  { /* Expression expr = parse_expression(0,reached+1,en-2); 
		...
		{ System.out.println("!! Invalid stereotype expression: " + showLexicals(reached+1,en-2)); 
		  checkBrackets(reached+1,en-2); 
	      Vector error5 = new Vector(); 
		  error5.add("Invalid stereotype"); 
		  error5.add(showLexicals(reached+1,en-2)); 
      ...
      { String p = lexicals.get(j+1) + ""; 
        ...
        { // System.out.println(lexicals.get(k) + ""); 
          ...
          { Type ptype = parseType(j+3,k-1,entities,types); 
            ...
            { System.out.println("!! Invalid parameter type: " + showLexicals(j+3,k-1)); }
          } 
        } 
        j++;  
      }
      ...
      { String p = lexicals.get(j+1) + ""; 
        ...
        { // System.out.println(lexicals.get(k) + ""); 
          ...
          { Type ptype = parseType(j+3,k-1,entities,types); 
            ...
            { System.out.println("!! Invalid attribute type: " + showLexicals(j+3,k-1)); }
          } 
        } 
        j++;  
      }
      ...
      { for (int k = j+1; k <= en; k++)
        { // System.out.println(lexicals.get(k) + ""); 
          ...
          { Expression expr = parse_expression(0,j+1,k-1,entities,types); 
            ...
            { System.out.println("!! Invalid precondition expression: " + showLexicals(j+1,k-1)); }
		} 
         } 
         j++;  
       }
  ...
  { // usecase name : type { parameters postconditions activity }
    ...
    if (foundstart == false) 
    { System.out.println("!! ERROR: use case syntax is: usecase " + nme + " : type { ... }");
      return uc;
    }
    ...
    { parseUseCaseParameters(uc,st+4,startactivity-2,entities,types);
	  ...
	  { checkBrackets(startactivity,actualend); 
	    System.out.println("!! Invalid statement at: " + showLexicals(startactivity,actualend)); 
      }
    }
      ...
      { for (int j = i+1; j < en; j++) 
        { if (":=".equals(lexicals.get(j) + ""))
          { Expression init = 
            ...
            { System.out.println("!! Error: Unrecognised type at " + showLexicals(i+1,j-1)); }  
          } 
        } 
        ...
        { System.out.println("!! Error: Unrecognised type at " + showLexicals(i+1,en-1)); }  
      }  
    } 
...
"(".equals(rlex))
  ...
  { if (e == s)
    ...
    { Expression ret = parse_ATLexpression(0,s+1,e,entities,types); 
      ...
      { System.out.println("Invalid expression in return: " + showLexicals(s+1,e)); }    
    } 
    else if ("catch".equals(lexicals.get(s) + "") && 
             "(".equals(lexicals.get(s+1) + ""))
    { for (int p = s+2; p < e; p++) 
              ...
              { for (int q = p-1; q > t+4; q--) 
                { String qs = lexicals.get(q) + ""; 
                  ...
                  { Vector params = 
                    ...
                    if (defn == null) 
                    { System.out.println("!! Invalid ATL expression: " + showLexicals(p+1,en-1)); 
                      return null; 
                    }
                    BehaviouralFeature bf = 
...
{ if ("using".equals(lexicals.get(st) + "")) {}
    ...
    { String var = lexicals.get(k-1) + "";
      ...
      { String mx = lexicals.get(m) + "";
        ...
        { Type typ = parseType(k+1,m-1,entities,types);
         if (typ == null)
         { System.out.println("!! Invalid type: " + showLexicals(k+1,m-1));
           continue;
         }
  ...
  { String rname = ""; 
    ...
    { start = st + 6; 
      ...
      if (inent == null) 
      { System.out.println("*** Cannot find entity type " + intype); 
  ...
  { if ("to".equals(lexicals.get(st) + "")) { }
    ...
    { String attname = lexicals.get(j) + ""; 
      ...
      { System.out.println("*** Cannot find entity type " + tname); 
  ...
  { String sts = lexicals.get(st) + "";
    ...
    { System.out.println("!! QVT-R transformation must start with transformation name(parameters)"); 
      return null; 
    }
    ...
    { String wlex = lexicals.get(w) + ""; 
      ...
      { // key eId { attname } ; 
        ...
        if (ent == null) 
        { System.out.println(">!! Key declaration error: No entity exists with name: " + eId); 
          st = w+4;
          start = w+6;  
          break; 
        }  
    ...
    { String lex = lexicals.get(i) + "";
      ...
      { Object r = parse_QVT_rule(start,i-1,entities,types,res);
        ...
        { System.out.println(">>> Unrecognised QVT element between " + start + " and " + (i-1)); } 
      }
    }
      ...
      { for (int j = i-1; j > st+2; j--) 
        { String jstr = lexicals.get(j) + ""; 
          ...
          { resType = parseType(j+1,i-1,entities,types); 
            if (resType == null) 
            { System.out.println("!! Invalid result type for function " + nme); } 
            ...
            if (code == null) 
            { System.out.println("!! Invalid expression for function " + nme); } 
    ...
    { String ch = lexicals.get(i) + "";
      ...
      { if (primitive) 
        { Type tt = parseType(st+4,i-1,entities,types); 
          if (tt == null) 
          { System.out.println("!! Invalid type at " + lexicals.get(start)); } 
        ...
        { TemplateExp cexp = parse_collection_exp(0, start, i-1, entities,types);
          if (cexp == null) 
          { System.out.println("!! Syntax error in domain " + nme); 
            continue; 
          } // should not happen.           
  ...
  { // bindsTo : Set(T) { att1 ++ att2 } { optionalExpression }
    ...
    { String ename = lexicals.get(st + 4) + "";
      ...
      if (e == null) 
      { System.out.println("!! ERROR: no class called " + ename); 
        return null; 
      } 
  ...
  { Vector res = new Vector();
    ...
    { String str = lexicals.get(i) + "";
      ...
      // distinguish being in a string and not. 
      // Within a string characters are copied, no extra
      // spaces. 

      if ("\\".equals(str) && i + 1 <= en && 
          "\"".equals(lexicals.get(i+1) + ""))
      { content = content + "\\\""; 
        i = i+1;
        continue;  
  ...
  { // Pre: en >= st + 3
	...
	// System.out.println("!! Not a word: " + showLexicals(st,en));
    return null; 
  }
  ...
  { String lex0 = "" + lexicals.get(st); 
	...
	{ String tag = "" + lexicals.get(st+1); 
	  ...
	  if (phs == null) 
	  { // System.out.println("!! Not a phrase list: " + showLexicals(st+2,en-1)); 
	    return null; 
	  }
	  else 
  ...
  { String lex0 = "" + lexicals.get(st); 
	...
	{ String tag = "" + lexicals.get(st+1); 
	  ...
	  if (sent == null) 
	  { System.out.println("!! Error: Not a sentence: " + showLexicals(st+2,en-1)); 
	    return null; 
       }
       return sent; 
     } 
  ...
  { // System.out.println(Double.MAX_VALUE); 
    ...
    // c.nospacelexicalanalysis("lambda x : int in (var y : int := x*x; return y)"); 
